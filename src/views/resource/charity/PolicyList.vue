...
import { 
  ...
  updatePolicy,
  deletePolicy
} from '@/api/resource/charity/policy'
import type { CharityPolicy } from '@/api/resource/charity/policy'
import PolicyDialog from './components/PolicyDialog.vue'
import PolicyViewDialog from './components/PolicyViewDialog.vue'

const loading = ref(false)
const tableData = ref<CharityPolicy[]>([])
...
const dialog = reactive({
  visible: false,
  isEdit: false,
  data: null as CharityPolicy | null
})
...
const handleEdit = async (row: CharityPolicy) => {
  try {
...
const handleView = async (row: CharityPolicy) => {
  try {
    const res = await getPolicyDetail(row.id)
    if ((res as any).code === 200 || (res as any).code === 0) {
...
const handleDelete = async (row: CharityPolicy) => {
...
const submitData = async (formData: CharityPolicy) => {
  try {
