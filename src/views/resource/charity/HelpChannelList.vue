...
>({
  ...
  fetchApi: async (params) => {
    const res = await getHelpChannelList(params)
    return res.data
  },
  errorMessage: '加载求助通道列表失败',
})
...
const {
  ...
  openView,
  handleDelete,
} = useCrudDialog<HelpChannelItem>({
  strategy: 'byId',
  getRowId: (row) => row.id,
  deleteItem: (row) => deleteHelpChannel(row.id),
  deleteMessage: (row) => `确认删除「${row.name}」吗？`,
  onSuccess: refresh,
})
    ...
    <el-card shadow="never" class="search-card">
      ...
      <el-form :inline="true" :model="query">
        ...
        <el-form-item label="通道类型">
          <el-select v-model="query.channelType" placeholder="全部" clearable style="width: 180px">
